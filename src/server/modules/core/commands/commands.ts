...
commandHandler.registerCommand({
  name: 'commands',
  handler: (invoker) => {
    const definitions = Object.entries(commandHandler.getCommandDefinitions())
      .filter(([commandKey, definition]) => commandKey === definition.name && (invoker.admin || !definition.admin))
      .sort((a, b) => a[0].localeCompare(b[0]));
    invoker.emitTo(`<G>Commands\n<B>${'-'.repeat(30)}\n<n>${definitions.map(([command]) => command).join('\n')}`);
  },
});
