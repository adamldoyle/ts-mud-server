import { commandHandler, parseArguments } from '@core/commands/CommandHandler';
import { buildZonedKey, catalog } from '@core/entities/catalog';
...
commandHandler.registerCommand({
  ...
  admin: true,
  handler: (invoker, command) => {
    const room = catalog.lookupRoom(command.rest, invoker.room.zone);
    if (!room) {
...
commandHandler.registerCommand({
  ...
  handler: (invoker, command) => {
    ...
    const character = catalog.loadCharacter(command.rest, invoker.room.zone, invoker.room);
    if (!character) {
...
commandHandler.registerCommand({
  ...
  handler: (invoker, command) => {
    ...
    try {
      const item = catalog.loadItem(itemKey, invoker.room.zone, { modifications });
      if (item) {
